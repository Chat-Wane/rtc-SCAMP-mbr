 ...
 */
Peer.prototype.onSubscription = function(uid, counter, offer){
    // #1 initialize the length of the walk to reach a random node to contact
    var hop = 2*this.partialView.length;
    this.onForwardedSubscription(uid, counter, offer, hop);
};
...
Peer.prototype.onForwardedSubscription = function(uid, counter, offer, hop){
	...
	for (var i = 0; i < this.partialView.length; ++i){
	    wout += this.partialView[i].weight;
	};
	// #1b normalize each outgoing arcs
	for (var i = 0; i < this.partialView.length; ++i){
	    this.partialView[i].weight = this.partialView[i].weight / wout;
	};
	// #2 choose the node to forward the subscription
	var rn = Math.random();
	var cumulativeProbability = 0;
	var i = 0;
	while ((cumulativeProbability + this.partialView[i].weight)<rn){
	    cumulativeProbability += this.partialView[i].weight;
	    ++i;
	};
	// #3 decrement the hop number and forward it to the proper peer
	hop -= 1;
	this.send(new SubscriptionRequest(uid,counter,offer,hop),
		  this.partialView[i].link);
    } else {
...
Peer.prototype.updateWeights = function(){
    ...
    for (var i = 0; i<this.inView.length; ++i){
	this.inView[i].weight = this.inView[i].weight / win;
	this.send(new MWeightUpdate(this.track.local.e,
				    this.inView[i].weight),
		  true,
		  this.inView[i].link);
    };
...
Peer.prototype.receive = function(link, message){
    // #0 (TODO) check if we already received this message in the past
    ...
    switch(message.type){
    case "MOfferResponse": break;
    case "MOfferRequest": break;
    case "MWeightUpdate":
	if (this.isFromInView){
	    var position = this.partialView.binaryIndexOf(message.uid);
	    this.partialView[position].weight = message.weight;
	} else {
	    var position = this.inView.binaryIndexOf(message.uid);
	    this.inView[position].weight = message.weight;
	}
    case "MSubscriptionRequest":
	// #A the message is a forwarded subscription
//	this.onForwardedSubscription(message.uid,
//				     message.counter,
//				     message.offer);
	break;
	...
	console.log("Peer "+message.uid+" is alive! Good to know :3");
	break;
    default:
...
Peer.prototype.broadcast = function(message){
    ...
    for (var i = 0; i<this.partialView.length; ++i){
	this.send(message, this.partialView[i].link);
    };
};
 ...
 */
Peer.prototype.onSubscriptionTODO = function(offers){
	...
	for (var i = 0; i<Math.min(offers.length,this.partialView.length);++i){
	    this.partialView[i].link.send(offers[i]);
	};
    };
};
 ...
 */
Peer.prototype.onForwardedSubscriptionTODO = function(uid, counter, offer){
    var self = this;
	...
	var offer = this.generateOffers(1, "response")[0];
	this.sub[offer.counter].destUid = uid;
	this.sub[offer.counter].destCounter = counter;
	var link = this.pending[offer.counter];
