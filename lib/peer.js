...
Peer.prototype.onContact = function(uid, counter, offer){
    ...
    setTimeout(function(){self.emit("offer",self.sub[firstLink.index]);},
	      Conf.waitSTUN);
};
...
Peer.prototype.receive = function(link, message){
	...
	// #0a forwarded subscription
	if (message.type == "MSubscriptionRequest"){
	    console.log("I HAZ REQUEST");
	...
	for (var i = 0; i<message.offers.length; ++i){
	    var pair = {_e:message.offers[i].uid,_c:message.offers[i].counter};
	    console.log(pair);
	    if (!this.backtrack.has(pair)){
		this.backtrack.set(pair,link);
	    };
	...
	if (message.destUid == this.track.local.e){
	    console.log(message.destUid + " ;;; ; ; " + this.track.local.e);
...
Peer.prototype.onSubscription = function(offers){
	...
	for (var i = 0; i<Math.min(offers.length,this.partialView.length);++i){
	    console.log("FORWARDED TO: "+this.partialView[i].uid);
