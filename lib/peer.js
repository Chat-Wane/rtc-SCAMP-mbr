...
function Peer(uid){
    ...
    this.track = new Track(uid); // for epidemic dissemination
 ...
 */
Peer.prototype.increment = function(){
    this.counter += 1;
 ...
 * network.
 */
Peer.prototype.initiate = function(){
    var self = this;
    var offer = this.generateOffers(1, "first")[0];
    setTimeout(function(){
	self.emit("first", offer);
    }, Conf.waitSTUN);
};
 ...
 */
Peer.prototype.generateOffers = function(k, type, destUid, destCounter){
    // #0 default values
...
Peer.prototype.onForwardedContact = function(uid, counter, offer, hop){
	...
	var offerContact = this.generateOffers(1, "contact", uid, counter)[0];
	...
	setTimeout(function(){
		...
		self.emit("contact", self.sub[offerContact.counter]);
	    } else {
 ...
 * cf the receive part of a broadcast).
 * \param message the message to broadcast
 */
Peer.prototype.broadcast = function(message){
    var tempMessage = message;
    if ((message.type === undefined) || (message.type!=="MBroadcast")) {
	var pair = this.track.increment();
	tempMessage = new MBroadcast(pair._e, pair._c, message);
    };
    this.sendToPartialView(tempMessage);
};

/*!
 * \brief send a message to its direct neighbourhood, i.e., through the link
 * in the partialView.
 * \param message the message to send to the partialView
 */
Peer.prototype.sendToPartialView = function(message){
    var i = 0;
...
Peer.prototype.onLeaseOver = function(){
