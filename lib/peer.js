...
function Peer(uid){
	...
	self.sendToPartialView(new MHeartbeat(uid));
    }, Conf.heartbeat);
...
Peer.prototype.receive = function(link, message){
	...
	break;
    case "Message":
	// #D the message is broadcast again
	console.log("received"+message.type+"; uid = "+message.uid);
	// (TODO) improve the stopping condition
...
Peer.prototype.broadcast = function(message){
	...
	var pair = this.track.increment();
	tempMessage = new Message(pair._e, pair._c, message);
    };
...
Peer.prototype.onForwardedSubscription = function(uid, counter, offer, hop){
    ...
    if ((!(uid == this.track.local.e)) && // not our own subscription request
